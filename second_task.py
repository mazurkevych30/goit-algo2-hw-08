...
class SlidingWindowRateLimiter:
    ...
    def _cleanup_window(self, user_id: str, current_time: float) -> None:
        ...
        while len(self.user_requests[user_id]) != 0:
            if current_time - self.user_requests[user_id][0] >= self.window_size:
    ...
    def time_until_next_allowed(self, user_id: str) -> float:
        ...
        if user_id not in self.user_requests or len(self.user_requests[user_id]) == 0:
            return 0.0

        wait_time = self.window_size - (current_time - self.user_requests[user_id][0])
